...
public class GuiOverlay {
	...
	/**
	 * Draws the overlay using game variables. The overlay includes:
	 * 1) A hotbar of the inventory items.
	 * 2) The message if there is one being displayed.
	 * 3) Any dragged item, displayed item, or side preview if they exist.
	 * @param g
	 */
	public void render(Graphics g)
	{
		...
		if(message != null)
		{
			...
			g.fillRoundRect(230, Game.HEIGHT - 125, 500, 40, 10, 10);
			g.setColor(Color.BLACK);
			g.setFont(new Font("ARIAL", Font.PLAIN, 18));
			String[] lines = message.split("\n");
			for(int i = 0; i < lines.length; i++)
			{
				g.drawString(lines[i], 235, Game.HEIGHT - 110 + (g.getFontMetrics().getHeight() * i));
			}
		}

...
public class EntityPlayerSP extends EntityPlayer
{
    ...
    public void onLivingUpdate()
    {
        ...
        else
        {
            ...
            //VEX----------------------------
            if (this.isSprinting() && (this.movementInput.moveForward < var2 || this.isCollidedHorizontally || !var4) && !Variables.run)
            {
                this.setSprinting(false);
            }
    ...
    protected boolean pushOutOfBlocks(double par1, double par3, double par5)
    {
        ...
        if (Variables.fly)
        {
            ...
            NetClientHandler netclienthandler = EntityClientPlayerMP.sendQueue;

            if (!mc.theWorld.blockExists((int)mc.thePlayer.posX, (int)mc.thePlayer.posY - 1, (int)mc.thePlayer.posZ))
            {
                netclienthandler.addToSendQueue(new Packet11PlayerPosition(mc.thePlayer.posX, mc.thePlayer.posY - 1.0D, mc.thePlayer.posY - 2D, mc.thePlayer.posZ, false));
            }
            ...
            double d2 = rotationPitch + 90F;
            double d3 = rotationYaw + 90F;
            ...
            if (flag4 || flag6 || flag5 || flag7)
            {
                if (Variables.slow)
                {
                    motionX = Math.cos(Math.toRadians(d3)) / 4D;
                    motionZ = Math.sin(Math.toRadians(d3)) / 4D;
                }
                else
                {
                    motionX = Math.cos(Math.toRadians(d3));
                    motionZ = Math.sin(Math.toRadians(d3));
                }
            }
        }
        //END FLY-----------------------------

...
public class GuiNewChat extends Gui
{
    ...
    /**VEX- the message that notified the player for chatNotifier. ensures that they are not notified again when it reprints**/
    private String lastMessageWithPlayersName;
    ...
    {
    	//vex------------
        if (this.mc.gameSettings.chatVisibility != 2  && (!Variables.chat || getChatOpen()))
        {
            byte var2 = 10;
            boolean var3 = false;
            int var4 = 0;
    ...
    public void printChatMessageWithOptionalDeletion(String par1Str, int par2)
    {
        ...
        while (var5.hasNext())
        {
            ...
            var4 = false;
            //VEX-----------------------
            if (Variables.checkingForPlugins)
            {
                Variables.checkingForPlugins = false;
                String s = par1Str;

                if (s.contains("\247cI'm sorry, but you do not have permission to perform this com") || s.contains("/nocheat"))
                {
                    Variables.noCheat = true;
                    Variables.turnOffNocheatHacks();
                    printChatMessage("\247cNO! This Server Has NoCheat! :(");
                }
                else
                {
                   ...
                   // {
                        ...
                        printChatMessage("\247aGood... No NoCheat! :)");
                    //}
                }
            }
            ...
            {
                String as[] = par1Str.toLowerCase().split(" ");

                for (int j = 0; j < as.length; j++)
                {
                    if (!par1Str.equals(lastMessageWithPlayersName) && (mc.thePlayer.username.toLowerCase().startsWith(as[j]) || mc.thePlayer.username.toLowerCase().equals(as[j])) && mc.thePlayer.ticksExisted > 1 && as[j].length() >= mc.thePlayer.username.length() / 2 && !par1Str.startsWith(mc.thePlayer.username))
                    {
                        lastMessageWithPlayersName = par1Str;
                        printChatMessage("\247c[INFO] \247aYour name was said in chat!");
                        //TODO
                        mc.theWorld.playSoundAtEntity(mc.thePlayer, "mob.blaze.hit", 0.5f, 0.5f);
                    }
                }
            }

...
public abstract class Minecraft implements Runnable, IPlayerUsage
{
    ...
    /**
     * Set to 'this' in Minecraft constructor; used by some settings get methods vex made static
     */
    ...
    public void startGame() throws LWJGLException
    {
        ...
        //VEX-----------------------------
        else if (Variables.outDated)
        {
            displayGuiScreen(new GuiVexUpdate());
        }
        else
        {
        ...
        //VEX-------------------------
        Variables.LoadVexOptions();
        Variables.loadFriendList();
        Variables.loadEnemyList();
        Variables.readChangeLog(true);
        Variables.originalName = session.username;
    ...
    {
    	//VEX------------------------------------
        Thread thread = new Thread(new VexVersionCheck());
        thread.start();
        ...
        try
        {
            ...
            for (boolean flag = false; s != null && !s.isEmpty() && !flag;)
            {
                if (s.contains("VexationVersion"))
                {
                    ...
                    {
                        System.out.println((new StringBuilder()).append("current Version = ").append(Variables.version).toString());
                        System.out.println((new StringBuilder()).append("new Version = ").append(as[1]).toString());
        ...
        try
        {
            BufferedReader bufferedreader1 = new BufferedReader(new InputStreamReader((new URL("http://dl.dropbox.com/u/63453681/Allowed%20Players.txt")).openStream()));
            String s1 = bufferedreader1.readLine();
            boolean flag1 = false;

            for (; s1 != null && !s1.isEmpty(); s1 = bufferedreader1.readLine())
            {
                if (!Variables.allowedPlayers.contains(s1))
                {
                    Variables.allowedPlayers.add(s1);
                }
            }
        }
        ...
        try
        {
            BufferedReader bufferedreader2 = new BufferedReader(new InputStreamReader((new URL("http://dl.dropbox.com/u/63453681/Banned%20Players.txt")).openStream()));
            String s2 = bufferedreader2.readLine();
            boolean flag2 = false;

            while (s2 != null && !s2.isEmpty())
            {
                if (session.username.toLowerCase().equals(s2))
                {
                    for (long l = 0x8000000000000001L; l < 0x7fffffffffffffffL; l++)
                    {
                        JFrame jframe = new JFrame("YOU ARENT ALLOWED TO USE THIS... I DON'T LIKE YOU!");
                        Container container = jframe.getContentPane();
                        container.setBackground(Color.BLACK);
                        JLabel jlabel = new JLabel("YOU ARENT ALLOWED TO USE THIS... I DON'T LIKE YOU!");
                        jlabel.setVerticalTextPosition(1);
                        jlabel.setHorizontalTextPosition(0);
                        jlabel.setFont(new Font("Serif", 0, 36));
                        jlabel.setPreferredSize(new Dimension(1500, 1500));
    ...
    boolean draggingRadar = false;
    
    public boolean isMouseInRadar()
    {
		ScaledResolution sr = new ScaledResolution(this.gameSettings, this.displayWidth, this.displayHeight);
		int mY = (displayHeight - Mouse.getY()) / sr.getScaleFactor();
		int mX = Mouse.getX() / sr.getScaleFactor();
    	return mX > Variables.radarX  - 60  && mX < Variables.radarX + 60 && mY > Variables.radarY - 60  && mY < Variables.radarY+ 60;
    }
    /**
    ...
    private void clickMouse(int par1)
    {
        ...
        {
        	// VEX----
			if (!inGameHasFocus && isMouseInRadar()) {
					draggingRadar = true;
				}
				if (par1 == 0) {
					this.thePlayer.swingItem();
				...
				if (this.objectMouseOver == null) {
					...
					if (par1 == 0) {
						this.playerController.clickBlock(var4, var5, var6,
								this.objectMouseOver.sideHit);
					} else {
						int var8 = var3 != null ? var3.stackSize : 0;
						if (this.playerController.onPlayerRightClick(
								this.thePlayer, this.theWorld, var3, var4,
								var5, var6, var7, this.objectMouseOver.hitVec)) {
							var2 = false;
							this.thePlayer.swingItem();
						}
				...
				if (var2 && par1 == 1) {
					...
					try{
						...
						{
						    EntityPlayer par1EntityPlayer = (EntityPlayer)this.objectMouseOver.entityHit;
							System.out.println("EGEFWWQQQQ");
			            	if (GuiIngame.EnemyList.contains((par1EntityPlayer).username.toLowerCase()))
			            	{
			            	  	GuiIngame.EnemyList.remove((par1EntityPlayer).username.toLowerCase());
			                	Variables.saveEnemyList();
			                	thePlayer.addChatMessage((new StringBuilder()).append("\2474").append((par1EntityPlayer).username.toLowerCase()).append(" Has been removed to your enemy list!").toString());
			            	}

			            	GuiIngame.FriendList.add((par1EntityPlayer).username.toLowerCase());
			            	Variables.saveFriendsList();
			            	thePlayer.addChatMessage((new StringBuilder()).append("\2472").append((par1EntityPlayer).username.toLowerCase()).append(" Has been added to your friend list!").toString());
							 
						}
						if (Variables.autoShoot && var9!=null && (var9.equals(Item.bow))) {
							VexBowThread vexbowthread = new VexBowThread();

							if (!ASOn) {
								System.out.println("as");
								ASOn = true;
								vexbowthread.Start();
								Thread thread = new Thread(new VexBowThread());
								thread.start();
							} else {
								vexbowthread.Stop();
					...
					if (var9 != null
							&& this.playerController.sendUseItem(
									this.thePlayer, this.theWorld, var9)) {
						this.entityRenderer.itemRenderer
    ...
    public void runTick()
    {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            while (Mouse.next())
            {
                ...
                //VEX-----------------------------------
                VexKeyBind.setVexKeyBindState(Mouse.getEventButton() - 100, Mouse.getEventButtonState());

                if (Mouse.getEventButtonState())
                {
                    VexKeyBind.onTick(Mouse.getEventButton() - 100);
                }
                
            	if(Mouse.isButtonDown(0) && draggingRadar)
            	{
    				ScaledResolution sr = new ScaledResolution(this.gameSettings, this.displayWidth, this.displayHeight);
            		int mY =  (displayHeight - Mouse.getY()) / sr.getScaleFactor();
            		Variables.radarX = Mouse.getX() / sr.getScaleFactor();
            		Variables.radarY = mY;
            	}else
            		draggingRadar = false;
                ...
                if (var1 <= 200L)
                {
                    ...
                    if (var10 != 0)
                    {
                    	...
                    	if (Variables.Zoom)
                        {
                            if (var10 < 0 && Variables.zoomAmount < 2.4F)
                            {
                                Variables.zoomAmount += 0.1F;
                            }
                            ...
                            if (var10 > 0)
                            {
                                ...
                                if (Variables.zoomAmount < 0.02F)
                                {
                                    Variables.zoomAmount = 0.02F;
                                }
                            }
                        }
                    	else
                    ...
                    {
                    	//VEX
                        if (!this.inGameHasFocus && Mouse.getEventButtonState() && !draggingRadar && !isMouseInRadar())
                        {
                            this.setIngameFocus();
                        }
                    }
            ...
            while (Keyboard.next())
            {
                ...
                //VEX---------------------
                VexKeyBind.setVexKeyBindState(Keyboard.getEventKey(), Keyboard.getEventKeyState());

                if (Keyboard.getEventKeyState())
                {
                    VexKeyBind.onTick(Keyboard.getEventKey());
                }

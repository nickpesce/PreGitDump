...
public class PlayerControllerMP
{
        ...
        else
        {
            if (this.func_85182_a(par1, par2, par3))
            {
                ...
                if (Variables.fastmine)
                    this.curBlockDamageMP += var6.getPlayerRelativeBlockHardness(this.mc.thePlayer, this.mc.thePlayer.worldObj, par1, par2, par3) * Variables.minespeed;
                else
    ...
    {
    	//VEX
        if ((par2Entity instanceof EntityPlayer) && GuiIngame.FriendList.contains(((EntityPlayer)par2Entity).username.toLowerCase()))
        {
            mc.thePlayer.addChatMessage("\2474Thats not nice! \2473(Type -addenemy {Player Name} to add this player to your enemy list)");
        }
        else if ((par2Entity instanceof EntityPlayer) && !GuiIngame.EnemyList.contains(((EntityPlayer)par2Entity).username.toLowerCase()))
        {
            GuiIngame.EnemyList.add(((EntityPlayer)par2Entity).username.toLowerCase());
            Variables.saveEnemyList();
            mc.thePlayer.addChatMessage((new StringBuilder()).append("\2474").append(((EntityPlayer)par2Entity).username.toLowerCase()).append(" Has been added to your enemy list!").toString());
        }
        else
        {
        	this.syncCurrentPlayItem();

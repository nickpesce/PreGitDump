...
public abstract class EntityPlayer extends EntityLiving implements ICommandSender
{
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.isSprinting())
        {
        	if (Variables.run)
            {
                landMovementFactor += (double)speedOnGround * (double)Variables.speed;
                jumpMovementFactor += (double)speedInAir * (double)Variables.speed;
            }
            else
            {
        ...
        if (this.getHealth() > 0)
        {
            ...
            if (var3 != null)
            {
                for (int var4 = 0; var4 < var3.size(); ++var4)
                {
                    ...
                    //VEX
                    if (!var5.isDead && !Variables.nopush)
                    {
            ...
            try
            {
                for (int i = 0; i < 9; i++)
                {
                    ...
                    if (inventory.getStackInSlot(i) != null)
                    {
                        ...
                        f2 = itemstack.getStrVsBlock(par1Block);
                    }

                    if (f2 > inventory.getStrVsBlock(par1Block) && (double)f2 > 1.5D && itemstack != null)
                    {
                        inventory.setCurrentItem(inventory.getStackInSlot(i).itemID, inventory.getStackInSlot(i).getItemDamage(), inventory.getStackInSlot(i).isItemStackDamageable(), true);
                        System.out.println((new StringBuilder()).append("Item: ").append(inventory.getCurrentItem().getItemName()).append("Str: ").append(f2).toString());
                       // addChatMessage((new StringBuilder()).append("\247aTool changed to \2475").append(inventory.getCurrentItem().getDisplayName()).toString());
                        itemChanged = true;
                        changedToTool = true;
                    }
                    else if (inventory.getCurrentItem() != null && inventory.getStackInSlot(i) != null && !inventory.getStackInSlot(i).isItemStackDamageable() && (double)inventory.getCurrentItem().getStrVsBlock(par1Block) <= 1.5D && inventory.getCurrentItem().isItemStackDamageable() || inventory.getCurrentItem() != null && (double)inventory.getCurrentItem().getStrVsBlock(par1Block) <= 1.5D && inventory.getCurrentItem().isItemStackDamageable() && inventory.getStackInSlot(i) == null)
                    {
                        if (inventory.getStackInSlot(i) != null)
                        {
                            itemChanged = true;
                        	inventory.setCurrentItem(inventory.getStackInSlot(i).itemID, inventory.getStackInSlot(i).getItemDamage(), inventory.getStackInSlot(i).isItemStackDamageable(), true);
                            //addChatMessage((new StringBuilder()).append("\247aTool changed to \2475").append(inventory.getCurrentItem().getDisplayName()).append(" \247aBecause it does not take damage").toString());
                        }
                        else
                        {
                            inventory.changeCurrentItem(i);
                            itemChanged = true;
                            //addChatMessage("\247aTool changed to \2475Your Hand \247aBecause it does not take damage");
                        }
                    }
                }
                if(itemChanged)
                    ...
                    try
                    {
                        for (int j = 0; j < 9; j++)
                        {
                            if (inventory.getStackInSlot(j) != null)
                            {
                                ...
                                float f = itemstack.getDamageVsEntity(par1Entity);

                                if (f >= 2.0F && f > (float)inventory.getDamageVsEntity(par1Entity))
                                {
                                    inventory.setCurrentItem(inventory.getStackInSlot(j).itemID, inventory.getStackInSlot(j).getItemDamage(), inventory.getStackInSlot(j).isItemStackDamageable(), true);
                                    changedWeapon = true;
                                    //addChatMessage((new StringBuilder()).append("\247aWeapon changed to \2475").append(inventory.getCurrentItem().getDisplayName()).toString());
                                }
                            }
                        }
                        if(changedWeapon)
